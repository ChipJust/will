...
- [ ] **Skill-vs-tool popup pattern — 3rd instance landed 2026-05-31, ready to write up.** When an agent-level workflow requires pre-steps before a tool call, those pre-steps should move INSIDE the tool, not stay in the docs. Demonstrated 3x: (1) clean_md graduation into ingest.py (2026-05-17); (2) YouTube `yt-dlp --get-title` baked into ingest.py (2026-05-24, retired 6 popups/batch); (3) PDF URL detection — `ingest.py` defaults to HTML extractor for URL inputs regardless of extension, so PDF URLs fail with "No article content extracted" until you curl-download locally and pass the file path (2026-05-31, retired 4 fallback steps for the snake-research batch). Write up `will/system/skill-tool-popup-pattern.md` alongside the tool-skill-pairs convention. Concrete fix for ingest.py: detect `URL.endswith('.pdf')` and check Content-Type after fetch, then route to the PDF extractor.
