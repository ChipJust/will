...
- [ ] `agent-scheduling` project: requirements/spec/design resolved 2026-04-26; ADRs 0001–0007 Accepted; code skeleton seeded at `projects/agent-scheduling/code/` with the first failing test. **Prototype target Jun 1, 2026.** Next is to commission the build (TDD walk through `05-implementation.md` slices). (from will session 2026-04-26)
- [ ] When Chip commissions the agent-scheduling overnight build, add pre-approvals to user-global `~/.claude/settings.json` for: `Bash(uv run python *)`, `Bash(uv run pytest *)`, `Bash(uv sync *)`, scoped to `D:/_code/will/projects/agent-scheduling/code/` if the harness supports prefix-scoping for these. Goal: no permission popups during the autonomous build. (from will session 2026-04-26)
- [ ] Decide if/when `agent-scheduling` graduates from `projects/` to its own repo (provisional name `convene`). Trigger candidates: ships to first real users; the `will`-as-meta-agent vs `will`-as-multi-user-platform conflation creates friction. (from will session 2026-04-26)
