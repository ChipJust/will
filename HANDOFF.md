...
- [ ] Provision frontend tooling for agent-scheduling phase 5 (PWA, slices 29–33). Decide framework (React, SolidJS, plain Web Components) and seed the structure when ready. (from will session 2026-04-26)
- [ ] User-global `~/.claude/settings.json` already covers `Bash(uv run:*)`, `Bash(uv sync:*)`, `Bash(uv add:*)`, `Bash(uv lock:*)` for the build. (Closed.)
