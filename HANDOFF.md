...
- [ ] **Rename `health-ingest` plugin to `ingest`.** It already carries the cross-repo `ingest-paper` skill (used by money) and is about to also carry `ingest-positions` (money-only). The "health-" prefix is misleading. 5-min rename + reinstall + update of any references in money/CLAUDE.md and reflections. (from money session 2026-04-27)
