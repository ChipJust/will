...
- [ ] **matplotlib.cm.get_cmap() removed** — any tool using matplotlib colormaps needs `plt.colormaps[name]` instead. May affect tools across repos. (from home session 2026-06-28)
- [ ] **Milestone execution pattern worth codifying** — plan workflow (survey → independent designs → adversarial critique → synthesized plan with binding decisions + gate-closure table) then implement workflow (sequential WPs, each implement → adversarial verify → fix → commit; repo green at every commit). Proven on spatium M1: planning 11 agents/~21 min; implementation 24 agents/~2.3 h; adversarial layers caught 26 design issues pre-code plus one physics error in the plan itself. Candidate for a will-level skill or workflow template once a 2nd milestone uses it (rule of 3 pending). (from spatium session 2026-07-02)
...
- [ ] **tasks.md pattern may be useful across repos.** Currently only in will. If a second repo needs tool-managed actionable items, graduate the wake-report task-checking to scan all repos. (from home session 2 2026-06-27)
- [ ] **Round-trip validation pattern** (ground truth → synthetic observations → reconstruction → error comparison) is a powerful testing methodology for any estimation pipeline. Worth noting as a reusable pattern if a 2nd project uses it. (from home session 2 2026-06-27)
- [ ] **Permission prompt pattern** — any tool that runs Bash/PowerShell commands repeatedly should be wrapped in a pre-approved Python script. This is a general ecosystem principle. Reference: Chip's explicit feedback in home session 2026-06-29. (from home session 2026-06-29)
- [ ] **MTP phone access via adb** — the phone_import tool uses Shell COM for MTP, which works for copy but not delete. Consider installing adb as a more reliable alternative for future sessions. (from home session 2026-06-29)
- [ ] **Worktree workflow merge gap** — fix agents in isolated worktrees don't auto-merge to the main repo. Either add an explicit merge phase to the workflow script or avoid worktrees for changes to the same files. (from home session 5 2026-06-29)
- [ ] **Coordinate convention documentation** — any tool doing 3D coordinate transforms should have a documented convention. This was the #1 source of bugs in home session 5. spatium solves this with a canonical frame (Y-up, meters) and pyproj for geographic transforms. (from home session 5 2026-06-29)
- [ ] **New repo: spatium** — standalone 4D world modeling system extracted from home's 3D pipeline. Needs to be added to the ecosystem table in PLAN.md and the wake report. Not yet on GitHub (local only at D:\_code\spatium\). (from spatium session 2026-06-30)
- [ ] **Round-trip testing as a convention** — the blueprint→model→blueprint pattern in spatium is a powerful verification strategy for any system with input/output formats. Worth documenting as a testing convention in `will/system/` after a 2nd project uses it. (from spatium session 2026-06-30)
- [ ] **/reflect should check `git status` of the working repo** — either commit leftovers or explicitly document why they're uncommitted. The 2026-06-29 home session reflected properly but left a dirty tree (small fix, regenerated PNGs, stale worktrees), which read as possible lost work a month later and required forensics. Add a working-tree check step to the reflect skill. (from home session 2026-07-26)
- [ ] **"Verify the artifact, not its data source"** — a generalizable testing lesson. The `home/tools/tree_reference.py` HTML passed every upstream check (all 19 Wikimedia URLs verified live from Python) and was still completely broken in the browser. Any generate-then-view tool must have the *view* step tested, not just its inputs. (from home session 2026-08-31)
- [ ] **`file://` blocks cross-origin `fetch()` in Chrome/Edge** — a hard constraint on this ecosystem's Python-generates-HTML pattern (already noted for the Three.js terrain viewer). Not a CORS-header issue; the browser refuses outright. **Embed data at build time (base64 data URIs), don't fetch at view time.** Reference impl: `home/tools/tree_reference.py` after the 2026-08-31 rewrite. Belongs in a tooling-conventions note. (from home session 2026-08-31)
- [ ] **Wikimedia API access rules, for any future reference-imagery tool** — `upload.wikimedia.org` returns `429 does not comply with our robot policy` unless the User-Agent follows `client/version (contact URL)`; use a repo URL, never a personal email. Batch API calls (50 titles max per request) rather than one-per-item, follow `normalized`/`redirects` maps to resolve titles, and back off exponentially on 429/503. (from home session 2026-08-31)
- [ ] **`Read` can't open PDFs on this machine** — requires `pdftoppm` / poppler-utils, not installed. `WebFetch` also failed on two PDFs (returned binary garble), so PDF sources are currently unreadable by default tooling. Worked around with a ~30-line zlib stream-decompress + text-operator extractor. **Either install poppler or graduate that extractor to `will/tools/pdf_text.py`** — it read two vendor manuals successfully. (from home session 2026-08-31)
- [ ] **Mid-turn user messages are a normal pattern, not an exception** — Chip interjected six or seven times while tools were running in the 2026-08-31 home session, each time with load-bearing facts. Worth noting in system conventions that long tool-running turns should expect and absorb them rather than finishing the original plan first. (from home session 2026-08-31)
- [ ] **Encoding conditionals into plan docs pays off later.** `garden-build.md`'s "4 trees need ≈60 ft, 5 need ≈75 ft — measure the run and the count falls out" let a locked decision be overturned by measurement as *executing the plan* rather than reversing it, with zero friction. Deliberately writing decision gates with their trigger conditions is a plan-authoring convention worth naming. (from home session 2026-08-31)
