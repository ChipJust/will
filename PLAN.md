...
| 2026-04-26 | Resolve `agent-scheduling` requirements/spec/design in one pass: scope expands beyond cohort to multi-context platform; multi-tenant cloud hosting; OAuth-first; calendar-read + email-send (no direct calendar write); directional per-edge privacy filter; full agent representation with invite-RSVP gate; batch-first negotiation; pluggable email/calendar adapters. ADRs 0001–0007 all Accepted. Code skeleton seeded at `projects/agent-scheduling/code/` with first failing test (slice 1 entry point). Prototype target Jun 1, 2026 |
| 2026-05-17 | Add `home` repo to ecosystem (life-domain stewardship — Austin Hill Country property). Graduate ingest pipeline (`tools/ingest.py` + `tools/extract/`) from health/money/home duplicates to `will/tools/` with deps in `will/pyproject.toml`. Rename plugin `health-ingest` → `ingest`. Pipeline now: `uv run --project D:/_code/will python D:/_code/will/tools/ingest.py <source>` from any subject repo cwd. Pattern: cwd-relative output, script-relative resources via `Path(__file__).parent`. |
